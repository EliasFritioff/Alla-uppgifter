...
def get_products(products): 
    ...
    for index, product in enumerate(products):
        product_info = f"{index + 1}. Product: {product['name']} \t {product['desc']} \t {locale.currency(product['price'], grouping=True)} \t Quantity: {product['quantity']}"
...
def get_product_by_id(products, product_id): 
    for index, product in enumerate(products):
        if product['id'] == product_id:
            return f"Product {index + 1}: {product['name']} \t {product['desc']} \t {locale.currency(product['price'], grouping=True)} \t Quantity: {product['quantity']}"
...
def view_products(products):
    for product in products:
        print(f"ID: {product['id']} - {product['name']} \t {product['desc']} \t {locale.currency(product['price'], grouping=True)} \t Quantity: {product['quantity']}")
        print()  # Lägg till en tom rad mellan varje produkt
